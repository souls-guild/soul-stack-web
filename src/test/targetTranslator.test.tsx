...
describe('targetTranslator', () => {
  it('empty spec → empty target, no warnings', () => {
  ...
  it('SIDs mode puts sids[] into target', () => {
  ...
  it('Coven mode puts coven[] into target', () => {
  ...
  it('escape string literal in CEL: double-quote and backslash', () => {
    const spec = makeSpec(['glob'], { glob: 'host"with\\quote' });
    const tr = translateTarget(spec);
    expect(tr.target.where).toBe('sid.glob("host\\"with\\\\quote")');
  });

  it('enabled mode with empty value → warning', () => {
