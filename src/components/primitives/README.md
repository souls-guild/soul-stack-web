...
Composite and prop names use the Soul Stack dictionary, never borrowed config-management jargon:

- `minion` → `Soul`
- `grain` → `Soulprint`
- `pillar` → `Essence`
- `state` (as a config DSL) → `Destiny`
- `pipeline/job` → `apply` / `scenario`
- `cluster` → `incarnation` (runtime instance) / `Coven` (label)
